...
def cleanningtext (data, both = True, onlyclean = False, dropduplicate = False):
    ...
    for i in range(0, len(data)):
        if both:
            tweet = tm.cleanText(str(data['content'][i]),fix=SlangS, pattern2 = True, lang = bahasa, lemma=lemmatizer, stops = stops, symbols_remove = True, numbers_remove = True, hashtag_remove=False, min_charLen = 2)
            tweet = tm.handlingnegation(tweet)
            print(i,tweet)
            cleantweet.append(tweet)
        elif onlyclean:
            tweet = tm.cleanText(str(data['content'][i]),fix=SlangS, pattern2 = True, lang = bahasa, lemma=lemmatizer, stops = stops, symbols_remove = True, numbers_remove = True, min_charLen = 3)
